...
def _get_current_app_dir():
    main_mod = sys.modules.get("main")
    if main_mod and hasattr(main_mod, "APP_DIR"):
        return getattr(main_mod, "APP_DIR")
    return APP_DIR
    ...
    def preload_templates(cls, directories=None, grayscales=(False, True)):
        ...
        with cls._preload_lock:
            if directories is None:
                cur_app_dir = _get_current_app_dir()
                directories = [
                    os.path.join(cur_app_dir, TEMPLATE_DIR_NAME),
                    os.path.join(cur_app_dir, FALLBACK_TEMPLATE_DIR_NAME),
                ]
    ...
    def start_adb_server(self):
        ...
        with self._device_lock:
            ...
            try:
                ...
                main_mod = sys.modules.get("main")
                adb_client_cls = getattr(main_mod, "AdbClient", AdbClient) if main_mod else AdbClient
                adb_device_cls = getattr(main_mod, "AdbDevice", AdbDevice) if main_mod else AdbDevice

                client = adb_client_cls(host=self.host, port=self.port)
                target = (
                    adb_device_cls(client, target_serial)
